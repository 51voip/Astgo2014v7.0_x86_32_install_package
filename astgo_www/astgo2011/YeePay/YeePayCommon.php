...
function getReqHmacString($p0_Cmd,$p2_Order,$p3_Amt,$p4_verifyAmt,$p5_Pid,$p6_Pcat,$p7_Pdesc,$p8_Url,$pa_MP,$pa7_cardAmt,$pa8_cardNo,$pa9_cardPwd,$pd_FrpId,$pr_NeedResponse,$pz_userId,$pz1_userRegTime,$pay_config)
{
	...
	$sbOld		=	$sbOld.$p0_Cmd;
	#加入商户代码
	$sbOld		=	$sbOld.$pay_config['yeepay_config_p1_MerId'];    # 替换为astgo 专用传递参数 $sbOld.$p1_MerId;
	#加入商户订单号
  ...
  #替换为 astgo 模式
	...
	logstr($p2_Order,$sbOld,HmacMd5($sbOld,$pay_config['yeepay_config_merchantKey']),$pay_config['yeepay_config_merchantKey']);
  return   HmacMd5($sbOld,$pay_config['yeepay_config_merchantKey']);


}


function annulCard($p2_Order,$p3_Amt,$p4_verifyAmt,$p5_Pid,$p6_Pcat,$p7_Pdesc,$p8_Url,$pa_MP,$pa7_cardAmt,$pa8_cardNo,$pa9_cardPwd,$pd_FrpId,$pz_userId,$pz1_userRegTime,$pay_config)
{
	...
	#调用签名函数生成签名串
	$hmac	= getReqHmacString($p0_Cmd,$p2_Order,$p3_Amt,$p4_verifyAmt,$p5_Pid,$p6_Pcat,$p7_Pdesc,$p8_Url,$pa_MP,$pa7_cardAmt,$pa8_cardNo,$pa9_cardPwd,$pd_FrpId,$pr_NeedResponse,$pz_userId,$pz1_userRegTime,$pay_config);
	...
	$params = array(
		...
		'p0_Cmd'						=>	$p0_Cmd,
		#加入商家ID
		'p1_MerId'					=>	$pay_config['yeepay_config_p1_MerId'],
    # 更改为astgo 模式 $p1_MerId,
		#加入商户订单号
		'p2_Order' 					=>	$p2_Order,
		#加入支付卡面额
		'p3_Amt'						=>	$p3_Amt,
  ...
  #替换为 astgo 模式
	$sNewString = HmacMd5($sbOld,$pay_config['yeepay_config_merchantKey']);
  logstr($r6_Order,$sbOld,HmacMd5($sbOld,$pay_config['yeepay_config_merchantKey']),$pay_config['yeepay_config_merchantKey']);
	...
	if($sNewString==$hmac) {
		...
		//echo "<br>YeePay:".$hmac;
		//echo "<br>交易签名无效!";
    return 'err1';// $pageContents;
		//exit;
	}
}

function generationTestCallback($p2_Order,$p3_Amt,$p8_Url,$pa7_cardNo,$pa8_cardPwd,$pa_MP,$pz_userId,$pz1_userRegTime,$pay_config)
{
	...
	$params = array(
		...
		'p0_Cmd'						=>	$p0_Cmd,
		#加入商家ID
		'p1_MerId'					=>	$pay_config['yeepay_config_p1_MerId'], # 更改为 aasgo $p1_MerId,
		#加入商户订单号
		'p2_Order' 					=>	$p2_Order,
		#加入支付卡面额
		'p3_Amt'						=>	$p3_Amt,
...
$p6_confirmAmount,$p7_realAmount,$p8_cardStatus,$p9_MP,$pb_BalanceAmt,$pc_BalanceAct,$pay_config)
{
	...
	$sbOld = $sbOld.$r0_Cmd;
	$sbOld = $sbOld.$r1_Code;
	$sbOld = $sbOld.$pay_config['yeepay_config_p1_MerId'];     #astgo 格式 $p1_MerId;
	$sbOld = $sbOld.$p2_Order;
	$sbOld = $sbOld.$p3_Amt;
...
$pc_BalanceAct,$hmac,$pay_config)
{
	if($hmac==getCallbackHmacString($r0_Cmd,$r1_Code,$p1_MerId,$p2_Order,$p3_Amt,
	$p4_FrpId,$p5_CardNo,$p6_confirmAmount,$p7_realAmount,$p8_cardStatus,$p9_MP,$pb_BalanceAmt,$pc_BalanceAct,$pay_config))
		return true;
	else
		return false;
